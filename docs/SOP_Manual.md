...
**目的**: 「完全に死んでいるポスト」を月1回特定し、ConoHa上のCronで安全に自動削除する。
判定は精度重視の全条件AND（削除は不可逆のため安全側に倒す）:
①投稿から48時間以上経過 ②6指標すべて0（いいね/返信/RT/ブックマーク/プロフクリック/詳細クリック）
③IMP < 中央値の50%。除外: 【保存版】等のホワイトリスト語を含む投稿・リプライ。
削除上限は**毎時最大2件・1日最大6件（DAILY_CAP）**。
