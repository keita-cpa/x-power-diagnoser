...
def generate_post(category, day_number, target_files=None, output_mode="tweet", knowledge_text=None, recent_topics=None, generate_reply=False, focus_theme=None):
    ...
    _NO_IMAGE_CATEGORIES = {"良客の目線・メンエス愛", "痛みの代弁・がんばりの承認", "趣味・人間味・日常", "X・SNS自虐/メタネタ"}
...
def evaluate_post(text, knowledge_text):
    ...
    eval_prompt = f"""# Role: あなたは徹底的な事実確認を行う「客観的真実の守護者」であり、Big4監査法人の厳格な品質管理（QC）パートナーです。あなたの最優先事項は正確性であり、創造性ではありません。

審査の鉄則（変更不可）：
・推測を一切しない。提示された[参考資料テキスト]と[生成された原稿]の記述のみを根拠とする。
・原稿の数字・条文番号・判例名を[参考資料テキスト]と一対一で照合して検証する。
・審査はステップバイステップで行い、以下の4基準を順番に判定する。

以下の【生成された原稿】が、プロフェッショナルとして世に出して問題ないか、厳しく審査してください。
...
基準4：外部リンク・ハッシュタグが含まれていないか。
（理由：2026年Xアルゴリズムでは外部リンクで配信50%削減ペナルティ、ハッシュタグはスパム信号として扱われる）
（判定：http://・https://・t.co/ 等のURLリンクが本文に1つでも含まれていれば基準4違反でリジェクト）
（判定：#記号で始まるハッシュタグ語（例: #税務調査 #メンエス）が含まれていれば基準4違反でリジェクト）
