...
| `tone_sample_01.txt` | トーン学習用サンプル — Read可（条件付き・下記「個人情報の保護」参照）/ Edit禁止 |
| `tone_sample_02.txt` | トーン学習用サンプル — Read可（条件付き・下記「個人情報の保護」参照）/ Edit禁止 |

これらは `.claude/settings.json` の `deny` リストでも保護されている
（tone_sample はユーザー承認 2026-06-12 により Read のみ解除）。
...
`tone_sample_01.txt` / `tone_sample_02.txt` はトーン校正用のサンプルデータ
（管理者確認済み: 2026-06-12、AI学習用に安全に作成・加工されたものと管理者が宣言）。
- Read許可（トーン分析・プロンプト作成目的のみ）。Edit禁止（データ改変防止）
- ただし以下は引き続き厳守:
  - 会話の相手方の発言を、生成物（プロンプト・投稿・ドキュメント）に逐語転載しない
  - 個人を特定しうる情報（名前・店舗名・日時・場所等）を生成物に書き出さない
  - 抽出するのは「文体の特徴」（テンポ・相槌・言葉選び）のみ。エピソード内容は抽象化する
...
"deny": [
  ...
  "Read(.env*)",           // 環境変数ファイル保護
  "Edit(.env*)",           // 同上
  "Edit(tone_sample*)",    // トーンサンプル改変防止（Readは2026-06-12にユーザー承認で解除）
  "Read(*.pem)",           // SSHキー保護
  "Edit(*.pem)"            // 同上
]
```
